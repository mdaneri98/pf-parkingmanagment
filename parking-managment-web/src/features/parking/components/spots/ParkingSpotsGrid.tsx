    ...
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-neutral-900 dark:text-neutral-100 mb-2">
          {t('parking.spots.noSpotsTitle')}
        </h3>
        <p className="text-sm text-neutral-500 dark:text-neutral-400">
          {t('parking.spots.noSpotsConfigured')}
        </p>
      </div>
    );
  }
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          ...
          >
            ...
            {t('parking.spots.previous')}
          </button>

          <div className="text-sm text-neutral-600 dark:text-neutral-400">
            {t('parking.spots.floorOf', { current: currentFloors[0], total: floors.length })}
          </div>
          ...
          <div key={floor} className="space-y-4">
            <div className="flex items-center space-x-2">
              ...
              <span className="px-2 py-1 text-xs bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 rounded-full">
                {t('parking.spots.spotsCount', { count: floorSpots.length })}
              </span>
            </div>
...
function SpotCard({ spot, onClick, icon, colorClass }: SpotCardProps) {
  ...
  return (
    <button
      ...
      title={`${spot.code} - ${spot.vehicleType} - ${spot.isAvailable ? t('parking.spots.available') : t('parking.spots.occupied')}`}
    >
      {/* Vehicle type icon */}
      ...
      {spot.isAccessible && (
        <span 
          title={t('parking.spots.accessible')}
          ...
          aria-label={t('parking.spots.accessible')}
        >
          ♿
        </span>
      )}
      {spot.isReservable && (
        <span 
          title={t('parking.spots.reservable')}
          ...
          aria-label={t('parking.spots.reservable')}
        >
          📅
        </span>
      )}
